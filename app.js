...
function renderStats() {
  ...
  for (let i = 0; i < days.length; i++) {
    ...
    const barH = Math.max(ratio * chartH, amt > 0 ? 4 : 0);
    const x = marginL + i * gap;
    const cy = chartH - barH + marginB;
    const cx = x + barW / 2;
    const isToday = ds === todayDateStr;

    bars += `<rect x="${x}" y="${cy}" width="${barW}" height="${barH}" rx="3" fill="#10B981" class="trend-svg-bar"/>`;

    if (amt > 0) {
      const val = fmt(amt).replace('¥', '');
      const fontSize = range === 'week' ? '9' : '7';
      values += `<text x="${cx}" y="${cy - 4}" text-anchor="middle" font-size="${fontSize}" fill="#059669" font-weight="500">${val}</text>`;
    }

    const label = range === 'week'
      ? ['日', '一', '二', '三', '四', '五', '六'][d.getDay()]
      : d.getDate().toString();
    labels += `<text x="${cx}" y="${baseline + 15}" text-anchor="middle" font-size="10" fill="${isToday ? '#059669' : '#9CA3AF'}" font-weight="${isToday ? '600' : '400'}">${label}</text>`;
  }
